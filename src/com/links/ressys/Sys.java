...
public class Sys {
	...
	public Reservation createReservation(String mailAddress, int[] roomIds, LocalDate startDate, LocalDate endDate,
			boolean onDb) {
	...
	public boolean validFidelityDate(int idReservation) {
		LocalDate today = LocalDate.now().plusDays(5);
		boolean fidelity = false;
		Iterator<Reservation> itReservation = reservationList.iterator();
		Reservation res = null;
		while (itReservation.hasNext()) {
			res = itReservation.next();
			if ((res.getReservationId() == idReservation) && (today.compareTo(res.getStartDate())<0)) {
				fidelity = true;
			} else {
				fidelity = false;
			}
